...
def main():
    ...
    if config.TRAIN.RESUME:
        ...
        if os.path.isfile(model_state_file):
            ...
            writer_dict['train_global_steps'] = 378 * 247
            writer_dict['valid_global_steps'] = 378 * 247
    ...
    print('Start training from epoch {} to {}'.format(last_epoch, end_epoch))
    for epoch in range(last_epoch, end_epoch):
        ...
        if args.local_rank == 0:
            ...
            torch.save({
                ...
                'epoch': epoch+1,
                'best_mIoU': best_mIoU,
                'state_dict': model.state_dict() if not is_parallel(model) else model.module.state_dict(),
            ...
            if mean_IoU > best_mIoU:
                best_mIoU = mean_IoU
                torch.save(model.state_dict() if not is_parallel(model) else model.module.state_dict(), os.path.join(final_output_dir, 'best.pth'))
            ...
            if epoch == end_epoch - 1:
                ...
                torch.save(model.state_dict() if not is_parallel(model) else model.module.state_dict(), final_model_state_file)
